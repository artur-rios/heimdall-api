...
| Public Id | A GUID generated when a top-level entity (`Scope`, `Role`, `Person`, `Application`, `Google User`) is created, used as its externally-facing identifier everywhere outside the database — API paths, response bodies, and authentication token claims |
...
Every top-level entity (`Scope`, `Role`, `Person`, `Application`, `Google User`) has two identifiers:
...
| Authentication | FR-AU-01 through FR-AU-07 |
