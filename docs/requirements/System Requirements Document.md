...
| FR-2F-03 | The email method shall deliver a random 6-digit numeric code through the existing email service (see FR-PR-02, FR-EV-02 for the established pattern), single-use and expiring 10 minutes after issue. The ten minutes is the tolerance this specification allows for mail delivery, which is the one part of the flow outside the system's control; no other lifetime in the second-factor flow may cut it short (NFR-17) | High |
...
| FR-2F-13 | The system shall count failed verification attempts against an issued email code and retire the code on the fifth, so a six-digit code cannot be exhausted by guessing. A further code may be obtained against an outstanding challenge without re-authenticating (FR-2F-16), at most **three** times per challenge — so one authentication attempt admits at most twenty guesses, a bound this specification states rather than one the rate limiter happens to impose. A fourth reissue shall require a fresh authentication attempt, as shall any reissue once the challenge itself has expired | High |
...
| POST | `/api/auth/2fa/challenge/resend` | Reissue the email code for an outstanding challenge, inside its existing window | No (holds a challenge token) |
...
| NFR-17 | Security | A two-factor challenge token shall carry a distinct claim marking it as MFA-pending, expire **10 minutes** after issue — a fixed lifetime, not a configurable one, and the same ten minutes FR-2F-03 gives the email code issued alongside it, so a login attempt has one deadline rather than two — and be rejected by every endpoint except second-factor verification |
...
NFR-05 used to promise a response time "under normal load" while nothing in the repository generated
any, so the clause could be neither met nor missed and the phrase was removed. It is back, with a
number behind it: `scripts/load_test.py` drives a deployment through
...
arrival. Correcting for that would mean building a benchmark harness, which is not what NFR-05 needs.
Compare these runs against each other across releases; do not publish them as a service level.

The run is deliberately not part of `dotnet test`. It needs a deployment, takes minutes, and its
figures depend on the machine it is driven from — none of which belongs in a suite that gates a
merge.

### 6.4 Accepted exception to NFR-15

**`Person.RoleId`.** `RoleId` is an internal `bigint` foreign key into
...
| Health & monitoring | FR-HC-01 through FR-HC-07 — specified in the [Operations & Infrastructure Document](Operations%20%26%20Infrastructure%20Document.md), §3.4, together with UC-30, since they describe how the system is run rather than what it does for an actor |
